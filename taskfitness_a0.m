...
% (sensory feedback strength to a0 neural pool)
...
% eps1_range = (4e-5:1e-5:2e-4);
% fsw_range = (0.005:0.01:0.1);

% bigger range but sparser points, will switch to LC mode for small eps1
% eps1_range = [(1e-6:2e-6:2e-5) linspace(2e-5,1e-3,10)];

% eps1_range = linspace(1e-3,1e-1,10); % extend eps1 to 0.1 
% fsw_range = (0.005:0.02:0.1);

% eps1_range = [(1e-6:2e-6:2e-5) linspace(2e-5,1,10)]; % extend eps1 to 0.1 
% fsw_range = (0:0.02:0.5);
% eps1_range = (1e-6:2e-6:6e-5);
eps1_range = (6e-5:4e-5:8e-4);
fsw_range = 0.045;
...
% seewead intake rate w.r.t. eps1 and fsw
figure
surf(eps1_range,fsw_range,-S0'); % need to add the negative sign! because:  -delta_x0 = seaweed intake
shading interp
% hold on; plot3(eps1_range(7),fsw_range(2),-S0(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_0$','interpreter','latex','fontsize',30)
ylabel('$\rm fsw$','interpreter','latex','fontsize',30,'rot',0)
...
subplot(2,2,3)
surf(eps1_range,fsw_range,T0'); % need to add the negative sign! because:  -delta_x0 = seaweed intake
shading interp
% hold on; plot3(eps1_range(7),fsw_range(2),T0(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_0$','interpreter','latex','fontsize',30)
ylabel('$\rm fsw$','interpreter','latex','fontsize',30,'rot',0)
...
subplot(2,2,4)
surf(eps1_range,fsw_range,delta_x0'); % need to add the negative sign! because:  -delta_x0 = seaweed intake
shading interp
% hold on; plot3(eps1_range(7),fsw_range(2),delta_x0(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_0$','interpreter','latex','fontsize',30)
ylabel('$\rm fsw$','interpreter','latex','fontsize',30,'rot',0)
