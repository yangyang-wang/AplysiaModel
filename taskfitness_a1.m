...
% perturbation on fsw: fsw -> fsw+eps as we vary fsw (load size) and eps2
% (sensory feedback strength to a1 neural pool)
...
% eps2_range = (4e-5:1e-5:2e-4);
% fsw_range = (0.005:0.01:0.1);

% in this range: model is still at HC mode
% eps2_range = [(1e-5:1e-5:1e-4) linspace(1e-4,1e-3,10)]; 
% eps2_range = [(1e-6:2e-6:2e-5) linspace(2e-5,1e-3,10)];
% eps2_range = linspace(1e-3,1e-1,10); % extend eps1 to 0.1 
% fsw_range = (0.005:0.02:0.1);

% eps2_range = linspace(1e-7,1e-3,20); % extend eps1 to 0.1 
% fsw_range = (0.0:0.02:0.1);

% eps2_range = [(1e-5:1e-5:2e-4) (2e-4:1e-4:1e-3)]; % extend eps1 to 0.1 
% fsw_range = (0:0.02:0.5);


eps2_range = [(1e-6:2e-6:6e-5) (6e-5:4e-5:8e-4)];
fsw_range = 0.045;

% eps2_range = linspace(5e-5,1e-2,10);
% fsw_range = 0.01;
...
for i=1:length(eps2_range)
    for j=1:length(fsw_range)
        ...
        % no perturbation, with parameters eps2 and fsw
        model1 = lyttle_model('tmax',20,'eps2',eps2); % assign new eps2 value
...
% save intake-rate.mat eps2_range fsw_range task_fitness_1storder task_fitness T0 Teps S0 Seps delta_x0 delta_xeps

%% Robustness/sensitivity to pertbation on fsw
figure
surf(eps2_range,fsw_range,task_fitness');
shading interp
% hold on; plot3(eps2_range(7),fsw_range(2),task_fitness(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_1$','interpreter','latex','fontsize',30)
...
% seewead intake rate w.r.t. eps2 and fsw
figure
surf(eps2_range,fsw_range,-S0'); % need to add the negative sign! because:  -delta_x0 = seaweed intake
shading interp
% hold on; plot3(eps2_range(7),fsw_range(2),-S0(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_1$','interpreter','latex','fontsize',30)
ylabel('$\rm fsw$','interpreter','latex','fontsize',30,'rot',0)
...
subplot(2,2,3)
surf(eps2_range,fsw_range,T0'); % need to add the negative sign! because:  -delta_x0 = seaweed intake
shading interp
% hold on; plot3(eps2_range(7),fsw_range(2),T0(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_1$','interpreter','latex','fontsize',30)
ylabel('$\rm fsw$','interpreter','latex','fontsize',30,'rot',0)
...
subplot(2,2,4)
surf(eps2_range,fsw_range,delta_x0'); % need to add the negative sign! because:  -delta_x0 = seaweed intake
shading interp
% hold on; plot3(eps2_range(7),fsw_range(2),delta_x0(7,2),'kp','MarkerEdgeColor','k','MarkerFaceColor',[1,0.5,0.5],'MarkerSize',15)
xlabel('$\varepsilon_1$','interpreter','latex','fontsize',30)
ylabel('$\rm fsw$','interpreter','latex','fontsize',30,'rot',0)
